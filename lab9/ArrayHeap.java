...
public class ArrayHeap<T> {
    ...
    private int min(int index1, int index2) {
        if (getNode(index1) != null && getNode(index2) != null) {
            if (Double.compare(getNode(index1).priority(), getNode(index2).priority()) < 0) {
                return index1;
            } else {
                return index2;
            }
    ...
    private void bubbleDown(int index) {
        while (getNode(index) != null && (getNode(index * 2) != null || getNode(index * 2 + 1) != null)) {
            if (getNode(min(index * 2, index *2 + 1)).priority() < getNode(index).priority()) {
                swap(index, min(index * 2, index * 2 + 1));
                index = min(index * 2, index * 2 + 1);
            } else {
                index = 0;
            }
        }
    ...
    public void changePriority(T item, double priority) {
        ...
        int pos = 1;
        while (pos <= size()) {
            if (getNode(pos).item().equals(item)) {
                index = pos;
            }
            pos += 1;
        }
